...
use reflect::msg::{
    CapitalizedResponse, CustomMsg, CustomMsgWrapper, CustomQueryWrapper, ExecuteMsg,
    InstantiateMsg, OwnerResponse, QueryMsg,
};
...
) -> Backend<MockApi, MockStorage, MockQuerier<CustomQueryWrapper>> {
...
fn proper_initialization() {
    ...
    let res: Response<CustomMsgWrapper> = instantiate(&mut deps, mock_env(), info, msg).unwrap();
...
fn reflect() {
    ...
    let _res: Response<CustomMsgWrapper> = instantiate(&mut deps, mock_env(), info, msg).unwrap();
    ...
    let res: Response<CustomMsgWrapper> = execute(&mut deps, mock_env(), info, msg).unwrap();
...
fn reflect_requires_owner() {
    ...
    let res: ContractResult<Response<CustomMsgWrapper>> = execute(&mut deps, mock_env(), info, msg);
...
fn reflect_subcall() {
    ...
    let info = mock_info("creator", &[]);
    let mut res: Response<CustomMsgWrapper> = execute(&mut deps, mock_env(), info, msg).unwrap();
    assert_eq!(0, res.messages.len());
