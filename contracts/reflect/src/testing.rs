use crate::msg::{CustomQueryWrapper, SpecialQuery, SpecialResponse};
...
) -> OwnedDeps<MockStorage, MockApi, MockQuerier<CustomQueryWrapper>> {
...
pub fn custom_query_execute(query: &CustomQueryWrapper) -> ContractResult<Binary> {
...
mod tests {
    ...
    fn custom_query_execute_ping() {
        let res = custom_query_execute(&CustomQueryWrapper {
    ...
    fn custom_query_execute_capitalize() {
        let res = custom_query_execute(&CustomQueryWrapper {
            route: "reflect".to_string(),
            query_data: SpecialQuery::Capitalized {
