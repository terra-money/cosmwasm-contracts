...
use crate::msg::{
    CallbackMsg, CapitalizedResponse, ChainResponse, CustomMsgWrapper, CustomQueryWrapper,
    ExecuteMsg, InstantiateMsg, OwnerResponse, QueryMsg, RawResponse, SpecialQuery,
    SpecialResponse,
};
...
pub fn instantiate(
    ...
    info: MessageInfo,
    msg: InstantiateMsg,
) -> StdResult<Response<CustomMsgWrapper>> {
    let state = State {
...
pub fn execute(
    ...
    info: MessageInfo,
    msg: ExecuteMsg,
) -> Result<Response<CustomMsgWrapper>, ReflectError> {
    match msg {
...
pub fn try_reflect_subcall(
    ...
    _env: Env,
    info: MessageInfo,
    msgs: Vec<SubMsg<CustomMsgWrapper>>,
) -> Result<Response<CustomMsgWrapper>, ReflectError> {
    let state = config(deps.storage).load()?;
    if info.sender != state.owner {
        return Err(ReflectError::NotCurrentOwner {
...
fn query_capitalized(deps: Deps, text: String) -> StdResult<CapitalizedResponse> {
    let req = CustomQueryWrapper {
        ...
        query_data: SpecialQuery::Capitalized { text },
    }
    .into();
...
fn query_chain(deps: Deps, request: &QueryRequest<CustomQueryWrapper>) -> StdResult<ChainResponse> {
...
mod tests {
    ...
    fn chain_query_works() {
        ...
        // with custom query
        let msg = QueryMsg::Chain {
            request: CustomQueryWrapper {
                route: "reflect".to_string(),
                query_data: SpecialQuery::Ping {},
            }
            .into(),
        };
        let response = query(deps.as_ref(), mock_env(), msg).unwrap();
