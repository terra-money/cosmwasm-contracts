...
use cosmwasm_std::{
    from_slice, log, CanonicalAddr, Env, HandleResponse, HumanAddr, InitResponse, Uint128,
};
...
fn get_owner<S: Storage, A: Api>(api: &A, storage: &S) -> HumanAddr {
...
fn init_works() {
    ...
    deps.with_storage(|storage| {
        ...
        assert_eq!(get_owner(&api, storage), address(4));
        Ok(())
    })
    .unwrap();
}
...
fn burn_works() {
    ...
    let api = deps.api;
    deps.with_storage(|storage| {
        assert_eq!(get_balance(&api, storage, &burner), 11);
        Ok(())
    })
    .unwrap();
    ...
    let env2 = mock_env_height(&address(4), 877, 0);
    ...
    // After
    deps.with_storage(|storage| {
        assert_eq!(get_balance(&api, storage, &burner), 10);
        Ok(())
    })
    .unwrap();
}
...
fn mint_works() {
    ...
    let api = deps.api;
    deps.with_storage(|storage| {
        assert_eq!(get_balance(&api, storage, &recipient), 11);
        Ok(())
    })
    .unwrap();
    ...
    let env2 = mock_env_height(&address(4), 877, 0);
    let mint_response: HandleResponse = handle(&mut deps, env2, mint_msg).unwrap();
    ...
    // After
    deps.with_storage(|storage| {
        assert_eq!(get_balance(&api, storage, &recipient), 12);
        Ok(())
    })
    .unwrap();
}
