...
use cosmwasm_std::{
    from_slice, log, Api, CanonicalAddr, Env, HumanAddr, ReadonlyStorage, StdError, Storage,
    Uint128,
};
use cosmwasm_storage::ReadonlyPrefixedStorage;

use crate::contract::{
    bytes_to_u128, handle, init, query, read_u128, Constants, KEY_CONSTANTS, KEY_MINTER,
...
fn get_minter<S: Storage>(storage: &S) -> CanonicalAddr {
    let config_storage = ReadonlyPrefixedStorage::new(PREFIX_CONFIG, storage);
    let data = config_storage
        .get(KEY_MINTER)
        .expect("no config data stored");
...
mod init {
    ...
    fn fails_for_symbol_too_short() {
        ...
        match result {
            Ok(_) => panic!("expected error"),
            Err(StdError::GenericErr { msg, .. }) => {
                assert_eq!(msg, "Ticker symbol is not in expected format [A-Za-z]{3,6}")
            }
            Err(e) => panic!("unexpected error: {:?}", e),
        }
    }
...
mod mint {
    ...
    fn fails_on_unauthorized() {
        ...
        let env2 = mock_env_height(&HumanAddr("owner0001".to_string()), 450, 550);
        let mint_result = handle(&mut deps, env2, mint_msg);
        match mint_result {
            Ok(_) => panic!("expected error"),
            Err(StdError::Unauthorized { .. }) => (),
            Err(e) => panic!("unexpected error: {:?}", e),
