...
use reflect::msg::{
    CustomMsg, CustomMsgWrapper, CustomResponse, HandleMsg, InitMsg, OwnerResponse, QueryMsg,
};
...
mod mock {
    use reflect::msg::{CustomQuery, CustomQueryWrapper, CustomResponse};
...
fn proper_initialization() {
    ...
    let res: InitResponse<CustomMsgWrapper> = init(&mut deps, env, msg).unwrap();
...
fn reflect() {
    ...
    let _res: InitResponse<CustomMsgWrapper> = init(&mut deps, env, msg).unwrap();
    ...
    let res: HandleResponse<CustomMsgWrapper> = handle(&mut deps, env, msg).unwrap();
...
fn reflect_requires_owner() {
    ...
    let res: HandleResult<CustomMsgWrapper> = handle(&mut deps, env, msg);
    match res {
